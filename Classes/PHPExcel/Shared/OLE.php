...
class PHPExcel_Shared_OLE
{
	...
	*/
	public function read($file)
	{
		$fh = fopen($file, "r");
		if (!$fh) {
			throw new PHPExcel_Exception("Can't open file $file");
		}
		$this->_file_handle = $fh;

		$signature = fread($fh, 8);
		if ("\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" != $signature) {
			throw new PHPExcel_Exception("File doesn't seem to be an OLE container.");
		}
		fseek($fh, 28);
		if (fread($fh, 2) != "\xFE\xFF") {
			// This shouldn't be a problem in practice
			throw new PHPExcel_Exception("Only Little-Endian encoding is supported.");
		}
