...
class PHPExcel_Shared_JAMA_Matrix {
			...
			switch($match) {
				...
				case 'array,integer':
						...
						if (($this->m * $this->n) == count($args[0])) {
							...
							throw new PHPExcel_Exception(self::ArrayLengthException);
						}
						break;
				default:
						throw new PHPExcel_Exception(self::PolymorphicArgumentException);
						break;
			}
		} else {
			throw new PHPExcel_Exception(self::PolymorphicArgumentException);
	...
	public function getMatrix() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				...
				case 'integer,integer':
						list($i0, $j0) = $args;
						if ($i0 >= 0) { $m = $this->m - $i0; } else { throw new PHPExcel_Exception(self::ArgumentBoundsException); }
						if ($j0 >= 0) { $n = $this->n - $j0; } else { throw new PHPExcel_Exception(self::ArgumentBoundsException); }
				...
				case 'integer,integer,integer,integer':
						...
						if (($iF > $i0) && ($this->m >= $iF) && ($i0 >= 0)) { $m = $iF - $i0; } else { throw new PHPExcel_Exception(self::ArgumentBoundsException); }
						if (($jF > $j0) && ($this->n >= $jF) && ($j0 >= 0)) { $n = $jF - $j0; } else { throw new PHPExcel_Exception(self::ArgumentBoundsException); }
				...
				case 'array,array':
						list($RL, $CL) = $args;
						if (count($RL) > 0) { $m = count($RL); } else { throw new PHPExcel_Exception(self::ArgumentBoundsException); }
						if (count($CL) > 0) { $n = count($CL); } else { throw new PHPExcel_Exception(self::ArgumentBoundsException); }
						$R = new PHPExcel_Shared_JAMA_Matrix($m, $n);
						for($i = 0; $i < $m; ++$i) {
							for($j = 0; $j < $n; ++$j) {
	...
	public function checkMatrixDimensions($B = null) {
		if ($B instanceof PHPExcel_Shared_JAMA_Matrix) {
			if (($this->m == $B->getRowDimension()) && ($this->n == $B->getColumnDimension())) {
				...
				throw new PHPExcel_Exception(self::MatrixDimensionException);
			}
		} else {
			throw new PHPExcel_Exception(self::ArgumentTypeException);
		}
	...
	public function plus() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				case 'object':
						if ($args[0] instanceof PHPExcel_Shared_JAMA_Matrix) { $M = $args[0]; } else { throw new PHPExcel_Exception(self::ArgumentTypeException); }
						break;
				case 'array':
	...
	public function times() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				case 'object':
						if ($args[0] instanceof PHPExcel_Shared_JAMA_Matrix) { $B = $args[0]; } else { throw new PHPExcel_Exception(self::ArgumentTypeException); }
						if ($this->n == $B->m) {
							...
							throw new PHPExcel_Exception(JAMAError(MatrixDimensionMismatch));
						}
						break;
				case 'array':
